...
2. *Showing the deterministic anchor* — "The Gemini extraction is bracketed on both sides. Pydantic schemas reject any unexpected field at ingress. The validation rules engine rejects any impossible port code, any negative rate, any validity window in the past. Every payload routes through Vertex AI with zero data retention. Cloud Run and storage residency stay in europe-west4; inference routes via the Vertex global endpoint while Gemini 3 europe-west4 GA is pending. Prospect data is destroyed at job completion regardless of routing path. Nothing the LLM extracts can land in your engine input without passing both gates."
