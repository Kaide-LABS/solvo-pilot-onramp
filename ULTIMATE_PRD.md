...
- **Xu & Lu — "TECP: Token-Entropy Conformal Prediction for LLMs"** — arXiv:2509.00461, Sept 2025. Develops a per-segment conformal apparatus driven by token-entropy rather than logprob, deliberately for open-ended generation where logprobs are sparse. *Why it validates our choice:* our section-granular conformal scoring needs a per-lane signal even when the Pro ensemble produces structured (low-token-count) JSON output with few logprobs per lane to integrate; TECP's token-entropy formulation generalizes cleanly to our N=3 majority-vote disagreement signal as the calibration covariate.
